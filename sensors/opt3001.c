...
double opt3001_get_data(I2C_Handle *i2c) {
    ...
    if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
        ...
        if (I2C_transfer(*i2c, &i2cMessage)) {
            ...
            uint16_t lightBuffer = ((rxBuffer[0]<<8) | rxBuffer[1]);
            int16_t exponent = (int16_t)(lightBuffer >> 12) & 0xF;
            int16_t fraction = lightBuffer & 0xFFF;
            lux = 0.01 * pow(2, exponent) * fraction;
